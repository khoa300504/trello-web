...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMember }) {
  ...
  const cardMembers = board.FE_allUsers?.filter(u => cardMemberIds.includes(u._id))
  /**
  ...
  const handleUpdateCardMembers = (user) => {
    ...
    onUpdateCardMember(incommingMemberInfo)
  }
    ...
    <Box sx={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
      ...
      >
        <Box sx={{ p: 2, maxWidth: '260px', display: 'flex', flexWrap: 'wrap', gap: 1.5 }}>
          {board?.FE_allUsers.filter(u => u._id !== currentUser._id).map((_, index) =>
            <Tooltip title={_.displayName} key={index}>
              {/* Cách làm Avatar kèm badge icon: https://mui.com/material-ui/react-avatar/#with-badge */}
              <Badge
                onClick={() => {handleUpdateCardMembers(_)}}
                sx={{ cursor: 'pointer' }}
                overlap="rectangular"
                anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}
                badgeContent={cardMemberIds.includes(_._id) ? <CheckCircleIcon fontSize="small" sx={{ color: '#27ae60' }} /> : null}
              >
                <Avatar
